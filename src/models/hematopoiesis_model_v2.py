...
def simulate_hematopoiesis(initial_lt_hsc_count, initial_st_hsc_count, params, time_steps, sample_days, label_fraction, initial_lt_quiescent, initial_st_quiescent, seed=None,):
    ...
    lt_hsc_division_counts = []    # Initialize a list to track division counts for all LT-HSCs
    ...
    for t in range(time_steps):
        ...
        # # Debugging: Count labeled LT-HSCs and ST-HSCs during sampling
        # labeled_lt_hscs = sum(1 for cell in cells if cell.cell_type == "LT-HSC" and cell.is_labeled)
        # labeled_st_hscs = sum(1 for cell in cells if cell.cell_type == "ST-HSC" and cell.is_labeled)
        # print(f"Time {t}: Labeled LT-HSCs: {labeled_lt_hscs}, Labeled ST-HSCs: {labeled_st_hscs}")

        # Print progress every 10% of the simulation, but ensure the interval is at least 1
        progress_interval = max(1, time_steps // 10)
        if t % progress_interval == 0:
            print(f"Progress: {t}/{time_steps} time steps completed...")
        ...
        # Track division counts for all LT-HSCs
        for cell in cells:
            if cell.cell_type == "LT-HSC":
                lt_hsc_division_counts.append({
                    "unique_id": cell.unique_id,
                    "division_count": cell.division_count
                })
        ...
        if t in sample_times:
            ...
            counts["LT-HSC_removed"].append(lt_hsc_removed)  # Append LT-HSC removed count
            counts["ST-HSC_removed"].append(st_hsc_removed)  # Append ST-HSC removed count
